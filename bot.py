...
#Чтобы пинговать бота и он не спал
app = Flask('')
...
ALLOWED_USERS = [
    int(v) for k, v in os.environ.items() if k.startswith("user_")
]
...
def get_all_photos():
    return sorted([f for f in os.listdir(PHOTO_DIR) if f.endswith('.jpg')])
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Какой номер сейчас ищем?":
        await update.message.reply_text(
            f"📌 Сейчас нужен номер: {current_number:03d}")
        return None
        ...
        if current_number == 1:
            ...
            if os.path.exists(path):
                await update.message.reply_photo(
                    photo=open(path, 'rb'),
                    caption=f"Последний номер: {current_number-1:03d}")
                return None
            else:
                await update.message.reply_text(
                    "⚠️ Фотография последнего номера не найдена.")
                return None
...
@allowed_user_only
async def handle_add_number(update: Update,
            ...
            if user_id != update.effective_user.id:
                try:
                    await context.bot.send_message(
                        chat_id=user_id,
                        text=
                        f"✅ Найден номер {submitted_number:03d}\n➡️ Теперь ищем {next_number:03d}"
                    )
                except Exception as e:
                    print(f"Не удалось отправить сообщение {user_id}: {e}")

    await update.message.reply_text(f"✅ Номер {submitted_number:03d} добавлен",
                                    reply_markup=MAIN_MENU)
    return ConversationHandler.END
...
def main():
    ...
    app.add_handler(
        MessageHandler(
            filters.Regex(
                "^(Какой номер сейчас ищем\\?|Последний номер|Все номера)$"),
            handle_buttons))

    conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^Добавить номер$"), begin_add_number)
        ],
        states={
            WAITING_FOR_NUMBER: [
            ...
            WAITING_FOR_PHOTO:
            [MessageHandler(filters.PHOTO, handle_add_photo)],
        },
        fallbacks=[])
